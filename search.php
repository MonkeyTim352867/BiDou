...
if (!empty($q)) {
    if (substr($q, 0, 1) === '%23') {
        ...
        $sql = "SELECT c.*, u.user_name, u.avatar FROM contents c 
                ...
                WHERE json_extract(c.tag, '$[*]') LIKE :tag OR json_extract(c.`hide-tag`, '$[*]') LIKE :tag";
        ...
        switch ($type) {
            case 'all':
                ...
                $sql = "SELECT c.*, u.user_name, u.avatar FROM contents c 
                        JOIN users u ON c.author = u.uid 
                        WHERE c.title LIKE :q OR u.user_name LIKE :q";
                $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>搜索结果 - <?php echo htmlspecialchars($q); ?></title>
    <!-- Tailwind CSS -->
...
<body class="bg-gray-50 font-sans text-text-primary">
    ...
    <main class="container mx-auto px-4 pt-24 pb-16 flex flex-col md:flex-row gap-6 justify-center">
        <div class="w-full md:w-full animate-fade-in">
            <h1 class="text-[clamp(1.5rem,3vw,2.5rem)] font-bold text-text-primary leading-tight mb-2">搜索结果 - <span class="text-primary"><?php echo htmlspecialchars($q); ?></span></h1>
            <?php if (!empty($search_results)): ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    <?php foreach ($search_results as $result): ?>
                        <?php if ($type === 'user'): ?>
                            <div class="bg-white rounded-xl shadow-card p-4 relative flex">
                                <a href="./user.php?uid=<?php echo htmlspecialchars($result['uid']); ?>" class="mr-3 w-12 flex-shrink-0">
                                    <img src="<?php echo htmlspecialchars($result['avatar']); ?>" alt="<?php echo htmlspecialchars($result['user_name']); ?> 头像" class="w-12 h-12 object-cover avatar-round">
                                </a>
                                <div class="flex flex-col justify-between min-w-0">
                                    <h2 class="text-lg font-bold mb-1 username-wrap-ellipsis">
                                        <a href="./user.php?uid=<?php echo htmlspecialchars($result['uid']); ?>">
                                            <?php echo htmlspecialchars($result['user_name']); ?>
                                        </a>
                                    </h2>
                                    <p class="text-text-tertiary text-sm intro-ellipsis">
                                        <?php echo htmlspecialchars($result['introduction']); ?>
                                    </p>
                                </div>
                            </div>
                        <?php else: ?>
            ...
            <?php else: ?>
                <div class="bg-white rounded-xl shadow-card p-8 text-center">
                    ...
                    <p class="text-text-secondary mb-6">你搜索的内容可能不存在</p>
